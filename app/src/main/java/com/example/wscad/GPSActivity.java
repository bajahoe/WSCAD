/**
 * 실행과 동시에 위치를 조회하여 반환하는 액티비티 입니다.
 * 권한 설정을 어플 실행시에 시도하여 허용하지 않는다면 실행되지 않게 하세요
 * 권한 없이 실행 시 0, 0 조회 결과 없음이 발생합니다.
 * */
////132478143276843211329178432769832
...
public class GPSActivity extends AppCompatActivity {
    ...
    //여기부터는 GPS 활성화를 위한 메소드들 제발 활성화 해주세요!
    private void showDialogForLocationServiceSetting() {
        ...
        builder.setNegativeButton("취소", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int id) {
                dialog.cancel();
                Toast.makeText(gpsTracker, "위치 서비스 설정을 거부하셨습니다.", Toast.LENGTH_SHORT).show();
            }
        });
        builder.create().show();
    }
